...
[Link to Github repository](https://github.com/Boonalds/Lesson7_TeamZeven.git)
...
```{r Calculating RMSE, message=FALSE, include=TRUE, results='show'}
...
Then the RMSE's are calculated per classes.
```{r Calculating RMSE per class, message=FALSE, include=TRUE, results='show'}
...
```
Apparently some classes are predicted better then others.