...
# Load libraries:
library(sp) ## GITHUB PW: team7en
library(raster)

# Load source scripts:
source("R/calculate.RMSE.R")
source("R/mask.trainingdata.R")
source("R/subset.perclass.R")
...
# Subsetting per class
ori_crop <- subset.perclass(trainingOri,1)
ori_for <- subset.perclass(trainingOri,2)
ori_wet <- subset.perclass(trainingOri,3)

pred_crop <- subset.perclass(trainingPred,1)
pred_for <- subset.perclass(trainingPred,2)
pred_wet <- subset.perclass(trainingPred,3)

# Calculating RMSE per class
RMSE_crop <- calculate.RMSE(ori_crop, pred_crop)
RMSE_for <- calculate.RMSE(ori_for, pred_for)
RMSE_wet <- calculate.RMSE(ori_wet, pred_wet)

paste("RMSE of cropland:",RMSE_crop)
paste("RMSE of forest:",RMSE_for)
paste("RMSE of wetland:",RMSE_wet)